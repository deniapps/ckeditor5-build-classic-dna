...
export default class CodeBlockEditing extends Plugin {
  ...
  init() {
    ...
    const normalizedLanguagesDefs = getNormalizedAndLocalizedLanguageDefinitions(
      editor
    );
    ...
    editor.commands.add(
      "outdentCodeBlock",
      new OutdentCodeBlockCommand(editor)
    );
    ...
    schema.register("codeBlock", {
      allowWhere: "$block",
      isBlock: true,
      allowAttributes: ["language", "codeID"],
    });
  ...
  afterInit() {
    ...
    this.listenTo(editor.editing.view.document, "enter", (evt, data) => {
      const positionParent = editor.model.document.selection.getLastPosition()
        .parent;
...
function leaveBlockStartOnEnter(editor, isSoftEnter) {
  ...
  if (
    isSoftEnter ||
    !modelDoc.selection.isCollapsed ||
    !lastSelectionPosition.isAtStart
  ) {
    return false;
  }
